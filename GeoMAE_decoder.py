...
class MAE(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_loc = nn.Linear(decoder_dim, 20)
    ...
    def forward(self, img):
        ...
        #geo_pred = self.encoder.transformer(tokens)
        #loc = self.to_loc(geo_pred[:, 0, :])
        ...
        geo_decoder_tokens = decoder_tokens[:, :1] + repeat(self.decoder_pos_emb[:1, :], 'n d -> b n d', b=batch)

        un_decoder_tokens = decoder_tokens[:, 1:, :] + self.decoder_pos_emb[1:, :][unmasked_indices]
        ...
        decoder_tokens_all[batch_range, unmasked_indices] = un_decoder_tokens
        decoder_tokens_all[batch_range, masked_indices] = mask_tokens

        decoder_tokens_all = torch.cat((geo_decoder_tokens, decoder_tokens_all), dim=1)
        decoded_tokens = self.decoder(decoder_tokens_all)

        pred = self.out(self.to_pixels(decoded_tokens[:, 1:, :]))

        # splice out the mask tokens and project to pixel values
        pred_mask_tokens = decoded_tokens[:, 1:, :][batch_range, masked_indices]
        pred_pixel_values = self.out(self.to_pixels(pred_mask_tokens))
        
        loc = self.to_loc(decoded_tokens[:, 0, :])
...
class MaeBirds(LightningModule):
    ...
    def shared_step(self, batch, batch_idx):
        ...
        loss = self.loss(pm, gt) + 0.01*geo_loss
        #loss = geo_loss
